...
MYAPP.display = {
	hideGameStarter: function() {//fadeOut:渐没 (ms) 
	...
	showWinMessage: function() {
		MYAPP.timeOuts.push(
			setTimeout(function() {
				$('.win-message').fadeIn(500).children('p').text("玩家 " + MYAPP.turn +
					" 胜利!! :D ")
			}, 1500));
	},
...
MYAPP.game = {
	...
	playerTurn: function(square) {
		...
		if (box.text() === '' && MYAPP.gameInPlay && (MYAPP.turn === 1 || (MYAPP.turn === 2 && MYAPP
				.secondPlayer))) {
			box.text(symbol);
...
MYAPP.computer = {
	...
	doubleThreatChoice: function(choiceType) {
		...
		if (board[5] === currentSymbol && MYAPP.numFilledIn === 3) {
			if ((board[1] === opponentSymbol && board[9] === opponentSymbol) || (board[3] ===
					opponentSymbol && board[7] === opponentSymbol)) {
				move = this.emptySide();
			}
		}
